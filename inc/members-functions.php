...
//
// MIF BP Mentorship members functions
//
//
...
function mentorship_get_mentors( $user = false )
{
    ...
    $arr = get_user_meta( $user_id, 'mentorship_mentor' );
...
function mentorship_get_learners( $user = false )
{
    ...
    if ( ! $arr = wp_cache_get( $user_id, "mentorship_get_learners") ) {
        ...
        $arr = $wpdb->get_results( "SELECT user_id FROM $table WHERE meta_key='mentorship_mentor' AND meta_value=$user_id", ARRAY_A );
...
function mentorship_add_member( $mentor = false, $learner = false )
{
    ...
    $ret = add_user_meta( $learner_id, 'mentorship_mentor', $mentor_id );
...
function mentorship_remove_member( $mentor = false, $learner = false )
{
    ...
    $ret = delete_user_meta( $learner_id, 'mentorship_mentor', $mentor_id );
    wp_cache_delete( $mentor_id, "mentorship_get_learners" );
